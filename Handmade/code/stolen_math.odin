...
@(private="file") Safe :: false

general_atan2_lane_f32 :: proc (y, x: lane_f32) -> lane_f32 {
    return cast(lane_f32) general_atan2_lane_f64(cast(lane_f64) y, cast(lane_f64) x)
}

general_atan2_lane_f64 :: proc (y, x: lane_f64) -> lane_f64 {
    // The original C code:
    //   Stephen L. Moshier
    ...
    if open == lane_false_64 do return result
    
    fast_result := fast_atan2_lane_f64(y, x)
    conditional_assign_once(&open, lane_true_64, &result, fast_result)
    
    return result
}

// @note(viktor): These do not handle NaNs and Infinities
fast_atan2_lane_f32 :: proc (y, x: lane_f32) -> lane_f32 {
    return cast(lane_f32) fast_atan2_lane_f64(cast(lane_f64) y, cast(lane_f64) x)
}

fast_atan2_lane_f64 :: proc (y, x: lane_f64) -> lane_f64 {
    when Safe {
        assert(is_nan(y) | is_nan(x) == lane_false_64)
        assert(is_inf(y) | is_inf(x) == lane_false_64)
    }
    ...
    open := lane_true_64
    c0 := equal(y, 0)
    c1 := greater_equal(x, 0)
    c2 := ~sign_bit_f64(x)
    conditional_assign_once(&open, c0 &   c1 & c2,  &result, copy_sign_f64(0.0, y))
    conditional_assign_once(&open, c0 & ~(c1 & c2), &result, copy_sign_f64(PI, y))
...
conditional_assign_once :: #force_inline proc (open: ^$M, mask: M, dest: ^$D, value: D) {
...
acos_lane_f64 :: proc (x: lane_f64) -> lane_f64 {
    ...
    */
    ...
    dwords := transmute(lane_u64) x
    hx := cast(lane_u32) shift_right(dwords, 32)
    lx := cast(lane_u32) dwords & 0xffffffff
    ix := hx & 0x7fffffff
    ...
    if open == lane_false do return result
...
is_inf :: proc (x: lane_f64, sign := 0) -> lane_u64 {
    ...
    conditional_assign(equal(x*0.5, x) & greater_than(x, 0), &result, sign >= 0 ? lane_true_64 : lane_false_64)
    return result
}