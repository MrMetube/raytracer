...
traverse_tree_and_test_hits :: proc (nodes: [] Oct_Node($Value), ray_o, ray_d: lane_v3, min_t, max_t: lane_f32,
    ...
    for stack_lens != 0 {
        ...
        it_bounds := lane_member(it, "bounds", Rectangle3)
        ...
        first_value   := lane_gather(lane_member(it, "first_value", Node_Index))
        subnodes      := lane_member(it, "subnodes", [8] Node_Index)
    ...
    for values_read != values_write {
        ...
        value := lane_member(node, "value", Value)
